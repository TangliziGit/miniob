...
RC DefaultHandler::create_index(
    Trx *trx, const char *dbname, const char *relation_name, const char *index_name, int attr_num, char *attribute_name[MAX_NUM],bool is_unique)
{
    ...
    const Value *value, int condition_num, const Condition *conditions, int *updated_count)
{
  ...
  RC rc = filter.init(*table, conditions, condition_num);
  if (rc != RC::SUCCESS) {
    return rc;
  }

...
RC Table::create_index(Trx *trx, const char *index_name,int attribute_num,char* attribute_name[MAX_NUM], bool is_unique)
{
  ...
  std::vector<const FieldMeta *> filed_metas;
  for (int i = 0; i < attribute_num; i++) {
    const FieldMeta *field_meta = table_meta_.field(attribute_name[i]);
    if (!field_meta) {
      return RC::SCHEMA_FIELD_MISSING;
    }
    filed_metas.push_back(field_meta);
  }

  IndexMeta new_index_meta;
  RC rc = new_index_meta.init(index_name, filed_metas, is_unique);
  if (rc != RC::SUCCESS) {
    return rc;
  }
  ...
  rc = index->create(index_file.c_str(), new_index_meta, filed_metas);
  if (rc != RC::SUCCESS) {
    delete index;
...
RC Table::delete_record(Trx *trx, Record *record)
{
  ...
  if (rc != RC::SUCCESS) {
    ...
    if( rc != RC::SUCCESS){
          LOG_ERROR(
        "Failed to recover indexes record (rid=%d.%d). rc=%d:%s", record->rid().page_num, record->rid().slot_num, rc, strrc(rc));
    }
    return rc;
  }

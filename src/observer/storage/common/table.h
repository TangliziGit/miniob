...
public:
  ...
  RC remove(const char *path, const char *name, const char *base_dir);
  /**
   * 打开一个表
  ...
  RC create_index(Trx *trx, const char *index_name,int attribute_num,char* attribute_name[MAX_NUM],bool is_unique);
...
public:
  ...
  std::vector<Index *> all_indexs() const 
  {
    return indexes_;
  }
